...
| 14 | Trustworthy Agricultural Intelligence Under Data Sparsity: An Empirical Study of Evidence Availability, Uncertainty, and Reliability (redesigned 2026-09-17, see below) | Empirical | [`evidence-sparsity-reliability`](active_tests/evidence-sparsity-reliability/experiment_blueprint.md) (predecessor [`regional-equity-audit`](concluded/regional-equity-audit/experiment_blueprint.md), concluded) | | | Active — Phase 0's original scope built (5/5 modalities, `EvidenceQualityVector` scored, 2 real multi-year time series: price 157 obs, production 465 obs). 2026-09-19: **E1's first real forecast run** - annual (not yet 30-day, no modality has that granularity yet) next-year price forecasting, T0 (price alone) vs. T0+production, backtested on real held-out years for KE/RW x coffee/maize/tea (6 series). Aggregate: T0+production wins on all 4 metrics, but a follow-up mechanistic investigation found the aggregate is misleading - KE coffee's 2021 test year is a real, documented global price shock no national yield feature could predict (both models ~identical there); KE tea's directional-accuracy drop (1.00->0.60) traces to production's real long-term trend spuriously biasing a trendless, volatile price target; RW coffee's "improvement" is most plausibly a 2-point calm-test-window sample artifact, not a real effect. Real refined finding: whether added evidence helps depends on whether its real variation is causally connected to the target's variation over the test window - not "more evidence, more accuracy." Followed up same day: built a third real time series (weather, 272 observations, all 4 countries x 34 years 1991-2024, zero gaps - the cleanest modality yet) and re-ran E1 with a weather tier. T1 (price+weather) gives a real, meaningful jump in directional accuracy (0.489->0.594) and modest MAPE gain but worse MAE/RMSE than price alone; T3 (+production on top) doesn't uniformly improve on T1, consistent with production's earlier-found issues. Real lesson: different modalities help different metrics differently - no single blended verdict captures it. 2026-09-20: this experiment moved to its own repo ([AgroIntel-EastAfrica/thesis-lab](https://github.com/AgroIntel-EastAfrica/thesis-lab), `agrointel` now a git submodule), then moved into **Experiment E2 (calibration)** - the first real test of the pre-registered H3 criterion. Real, honest result: H3's specific threshold (uncalibrated misses by >20pts) was NOT met (only a 10.8pt miss, pooled across 26 real test points) - the concluded experiment's original 41.5pt gap came from a different, more complex production system, not this simpler baseline, so it was never guaranteed to reproduce. But the calibrated baseline (LOO-conformal) still measurably outperformed the naive one (6.9pt miss vs 10.8pt) - H3's direction holds even where its magnitude threshold doesn't. 2026-09-21: extended E2 to T1 (price+weather) - real, clear result: adding weather made calibration WORSE for both baselines (Baseline A: 10.8pt miss -> 18.5pt miss; Baseline D: 6.9pt -> 10.8pt), RW coffee's Baseline A coverage collapsing from 100% to 0%. Mechanistic explanation: 9-24 training points can't support 3-4 fitted parameters without real out-of-sample variance the training-time uncertainty estimates never anticipated - the same "more evidence isn't free" theme as E1, now shown for calibration via an independent mechanism (small-sample parameter variance, not spurious trend correlation or a macro shock). Real support for evaluating forecasting/calibration/evidence-quality as separate dimensions, not one blended score. Same day: **started Experiment E3 (evidence grounding)** - the blueprint's own "where Paper 14 becomes genuinely agentic" experiment. Built a real, mechanical evidence-state classifier + agent-action selector (sufficient->recommend; insufficient->retrieve then abstain; stale->recommend with caveat; conflicting/semantically-incompatible->investigate then caveat or abstain) and ran it against all 5 of the blueprint's named scenarios, every one grounded in real data or a real documented incident (not fabricated) - e.g. Scenario E used KE coffee's real $4,886.5/tonne price vs. real $518,720,901 trade value as the blueprint's own "different price bases" example, instantiated for real. Result: 5/5 scenarios matched their expected action. Honest limit stated plainly: this validates the decision procedure's internal consistency against 5 known, hand-picked cases - not yet that a real LLM-driven agent behaves this way, or that it generalizes to novel cases. Same day: **wrote E4's real study protocol** (`E4_study_protocol.md`) - E4 is a human-subjects study (decision accuracy, response time, override behavior across 5 presentation formats), which a coding session cannot execute; simulating fake participants would be fabrication. The protocol's worked scenario uses real numbers already computed by this experiment (KE coffee 2024: real 2023 price -> E1's T0/T1 forecasts -> E2's real 80% interval -> E3's real evidence-state classification -> the real 2024 actual price for scoring), ready to run rather than a placeholder, but genuinely not run - needs real participant recruitment, informed consent, and institutional review (Makerere/UBOS/MAAIF, per the source proposal), which is the advisor's/institution's call. **Status: all 4 sub-experiments (E1-E4) now touched** - E1/E2/E3 with real computed results, E4 with a real ready-to-run protocol. What remains is depth (more series, sparsity sweeps, the full E4 battery) and E4's actual human study, which this session cannot run alone |

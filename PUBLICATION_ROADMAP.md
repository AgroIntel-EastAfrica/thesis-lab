...
| 14 | Trustworthy Agricultural Intelligence Under Data Sparsity: An Empirical Study of Evidence Availability, Uncertainty, and Reliability (redesigned 2026-09-17, see below) | Empirical | [`evidence-sparsity-reliability`](active_tests/evidence-sparsity-reliability/experiment_blueprint.md) (predecessor [`regional-equity-audit`](concluded/regional-equity-audit/experiment_blueprint.md), concluded) | | | Active — Phase 0's original scope now built: all 5 planned evidence modalities collected live at least once (price/weather/production/trade clean; text reliability-limited, ~2 of 12 GDELT calls succeeding — a real evidence-*quality* finding), plus a first, real, individually-justified `EvidenceQualityVector` computed across all 56 observations (`evidence_quality_rubric.py`) — no modality wins on every dimension (weather: best source, worst geographic fidelity; trade: strong provenance, weakest compatibility; text: weakest almost everywhere). Not yet done: real time-series data (still single-snapshot), H1–H5 numeric success criteria. Next: a second collection pass, or moving into Experiment E1 |

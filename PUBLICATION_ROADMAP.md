...
| 14 | Trustworthy Agricultural Intelligence Under Data Sparsity: An Empirical Study of Evidence Availability, Uncertainty, and Reliability (redesigned 2026-09-17, see below) | Empirical | [`evidence-sparsity-reliability`](active_tests/evidence-sparsity-reliability/experiment_blueprint.md) (predecessor [`regional-equity-audit`](concluded/regional-equity-audit/experiment_blueprint.md), concluded) | | | Active — Phase 0's data-collection layer built: all 4–5 planned evidence modalities have at least one real, live collection (price: split by country tier; weather: uniform across all 4 countries; production: uniform across countries but varies by commodity — unblocked by fixing 3 real production bugs in `services/forecasting/yield_prediction.py`, not yet merged to master pending project-owner go-ahead; trade: real data for Kenya only, RW/SS/SO confirmed genuine zero, breaking the price-tier pattern since RW is otherwise "data-rich"; text/news: reliability-limited, ~2 of 12 GDELT calls succeeding across 3 live runs even after fixing a real concurrency bug — a genuine evidence-*quality*, not *availability*, finding). Five modalities, five different real shapes of "sparse" or "unreliable" — strong support for the core premise. Next: `EvidenceQualityVector` population and/or a second collection pass to start real time-series data |

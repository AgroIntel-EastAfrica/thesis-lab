...
def fig_production_history():
    ...
    for ax, comm in zip(axes, ["coffee", "maize", "tea"], strict=False):
        ...
        ax.legend(frameon=False, fontsize=8)
    fig.suptitle("Real FAOSTAT production/yield, 1961–2024 — maize tracked for all 4, coffee/tea only for KE/RW", fontweight="bold")
    fig.tight_layout()
...
def fig_weather_uniformity():
    ...
    for ax, var, label in zip(axes, ["temperature_2m", "precipitation_corrected"], ["Temperature (°C)", "Precipitation (mm/day)"], strict=False):
        ...
        ax.set_xlabel("Year")
        ax.legend(frameon=False, fontsize=8)
    fig.suptitle("Real NASA POWER weather, 1991–2024 — uniformly available across all 4 countries regardless of price-data tier", fontweight="bold")
    fig.tight_layout()
...
def fig_evidence_quality_radar():
    ...
    ax.set_ylim(0, 1)
    fig.suptitle("EvidenceQualityVector by modality — no modality wins on every dimension", fontweight="bold", fontsize=12, x=0.45)
